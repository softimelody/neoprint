...
export default function Register() {
    ...
    return (
        <div className={`min-h-screen flex flex-col font-sans transition-colors duration-700 relative overflow-hidden ${
            ...
            <Head title="Registrarse | NeoPrint" />
            ...
            <main className="flex-grow flex items-center justify-center p-4 z-10">
                ...
                <div className={`relative w-full max-w-md p-8 rounded-3xl border shadow-2xl backdrop-blur-xl transition-colors duration-700 ${
                    ...
                    <div className="text-center mb-8">
                        ...
                        <p className={`font-bold ${isAero ? 'text-slate-500' : 'text-slate-400'}`}>Únete a la nueva era del merch</p>
                    </div>

                    <form onSubmit={submit} className="space-y-5">
                        <div>
                            <label className={`block text-sm font-bold mb-2 ${isAero ? 'text-slate-700' : 'text-slate-300'}`}>Nombre de Usuario</label>
                            <input
                                id="name"
                                name="name"
                                value={data.name}
                        ...
                        <div>
                            <label className={`block text-sm font-bold mb-2 ${isAero ? 'text-slate-700' : 'text-slate-300'}`}>Confirmar Contraseña</label>
                            <input
                                ...
                                autoComplete="new-password"
                            />
                            {errors.password_confirmation && <p className="text-red-500 text-xs mt-1 font-bold">{errors.password_confirmation}</p>}
                        </div>

                        <button
                            disabled={processing}
                            className={`w-full mt-2 py-3.5 rounded-xl text-white font-black tracking-wide uppercase transition-all duration-300 hover:-translate-y-1 shadow-lg ${
                                isAero 
                        ...
                        <p className="text-center text-sm font-bold mt-4">
                            <span className={isAero ? 'text-slate-500' : 'text-slate-400'}>¿Ya eres miembro? </span>
                            <Link href={route('login')} className={`hover:underline ${isAero ? 'text-blue-600' : 'text-cyan-400'}`}>Inicia sesión</Link>
                        </p>
                    </form>
                </div>
            </main>
