...
export default function ForgotPassword({ status }) {
    ...
    return (
        <div className={`min-h-screen flex flex-col font-sans transition-colors duration-700 relative overflow-hidden ${
            ...
            <Head title="Recuperar Contraseña | NeoPrint" />
            ...
            <main className="flex-grow flex items-center justify-center p-4 z-10">
                ...
                <div className={`relative w-full max-w-md p-8 rounded-3xl border shadow-2xl backdrop-blur-xl transition-colors duration-700 ${
                    ...
                    <div className="text-center mb-6">
                        ...
                        <h2 className={`text-xl font-black ${isAero ? 'text-slate-800' : 'text-white'}`}>Recuperación de acceso</h2>
                    </div>

                    <div className={`mb-6 text-sm font-medium leading-relaxed text-center ${isAero ? 'text-slate-600' : 'text-slate-400'}`}>
                        ¿Olvidaste tu contraseña? No hay problema. Solo indícanos tu dirección de correo electrónico y te enviaremos un enlace para que puedas elegir una nueva.
                    </div>

                    {status && <div className="mb-4 font-bold text-sm text-center text-emerald-500">{status}</div>}

                    <form onSubmit={submit} className="space-y-5">
                        <div>
                            <label className={`block text-sm font-bold mb-2 ${isAero ? 'text-slate-700' : 'text-slate-300'}`}>Email registrado</label>
                            <input
                                id="email"
                                type="email"
                                name="email"
